...
class CheapRaster {
  public:
    ...
    // the solid texels (tint is the fill, flags and mask still apply — a
    // masked/blended solid is image(Texture::White, ...)). An unregistered
    // host id (>= Texture::FirstIndex) draws loud magenta in kReal.
