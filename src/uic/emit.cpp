...
std::string emitPanelHeader(const Module &m, const EmitOptions &opt,
  ...
  os << "#pragma once\n"
     ...
     << "  return buf;\n"
     << "}\n"
     << "// f is a format_string, checked against the args at COMPILE time — a\n"
     << "// plain string literal is constexpr at the call site, so no\n"
     << "// FMT_COMPILE is needed. Args are FORWARDING references: format_to\n"
     << "// re-deduces the arg types, and forwarding keeps Args exactly those\n"
     << "// re-deduced types, so f's format_string<Args...> stays that exact\n"
     << "// type. (By value the copies are lvalues -> format_to wants\n"
     << "// format_string<T&>, which MSVC's consteval check refuses to make.)\n"
     << "template <class... Args>\n"
     << "inline ::fmt::memory_buffer fmt(::fmt::format_string<Args...> f,\n"
     << "                                Args &&...args) {\n"
     << "  ::fmt::memory_buffer buf;\n"
  ...
  if (hierOut != nullptr) {
    ...
    // manifests, and UiSnapshot, so it must be included AFTER it. Re-emitting
    // the preamble would redefine those and fail to compile alongside it.
    std::ostringstream h;
    h << "#pragma once\n"
