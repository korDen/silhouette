  ...
  std::string expr(const Expr &e) {
    ...
    case Expr::kCall:
      ...
      if (e.args[0]->kind == Expr::kIdent && e.args[0]->text == "fmt") {
        ...
        std::string out =
            "gen_detail::fmt(FMT_COMPILE(" + quotedLit(e.args[1]->text) + ")";
...
std::string emitPanelHeader(const Module &m, const EmitOptions &opt,
  ...
  os << "#pragma once\n"
     ...
     << "// num(v), fixed(v, decimals), fmt(\"{} / {}\", ...) render into a\n"
     << "// {fmt} memory_buffer (512 bytes inline, heap only if a run\n"
     << "// overflows — never truncates). sv() borrows the run for the Sink's\n"
     << "// string-lifetime contract. `::fmt::` is the library namespace,\n"
     << "// disambiguated from this gen_detail::fmt.\n"
     << "inline ::fmt::memory_buffer num(long long v) {\n"
     << "  ::fmt::memory_buffer buf;\n"
     << "  ::fmt::format_to(std::back_inserter(buf), \"{}\", v);\n"
     << "  return buf;\n"
     << "}\n"
     << "inline ::fmt::memory_buffer fixed(double v, int decimals) {\n"
     << "  ::fmt::memory_buffer buf;\n"
     << "  ::fmt::format_to(std::back_inserter(buf), \"{:.{}f}\", v, decimals);\n"
     << "  return buf;\n"
     << "}\n"
     << "// f is a COMPILED format (FMT_COMPILE wraps the literal at the call\n"
     << "// site) forwarded with the args into format_to. A generic\n"
     << "// CompiledFormat parameter -- not format_string<Args...> -- is what\n"
     << "// lets the format pass through this wrapper: format_to re-deduces the\n"
     << "// arg types, and a format_string built here cannot survive that\n"
     << "// re-deduction (MSVC's consteval check), but a compiled format can.\n"
     << "// Args are forwarding references so their value categories reach\n"
     << "// format_to unchanged.\n"
     << "template <class CompiledFormat, class... Args>\n"
     << "inline ::fmt::memory_buffer fmt(CompiledFormat &&f, Args &&...args) {\n"
     << "  ::fmt::memory_buffer buf;\n"
     << "  ::fmt::format_to(std::back_inserter(buf),\n"
     << "                   std::forward<CompiledFormat>(f),\n"
  ...
  if (hierOut != nullptr) {
    // The walk header is MINIMAL: it reuses the panel header's gen_detail,
    // manifests, and schema include, so it must be included AFTER it.
    // Re-emitting the preamble would redefine those and fail to compile
    // alongside it.
    std::ostringstream h;
    h << "#pragma once\n"
      << "// GENERATED by uic --emit-hier from " << m.name << ".ui -- DO NOT "
         "EDIT.\n"
      << "// The structural parity walk. Include the panel's --emit header "
         "FIRST\n"
